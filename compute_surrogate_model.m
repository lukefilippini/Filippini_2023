function [s_exp,lambda,theta] = compute_surrogate_model(d,D,IB,OB,model,IF,tc)
...
if isscalar(D) % rate parameters for homogeneous media
    ...
    if L0 == 0 % rate parameters for disc/sphere
        if isequal(model,'one-term')
            lambda = d*(d+2)*D/(L1^2 + (d+2)*beta*L1);
        elseif isequal(model,'two-term')
        ...
        if isequal(model,'one-term') % single parameter exponential model
            ...
            lambda = zeros(2,1); % two parameters
            if isequal(model,'two-term')
                theta = 0.5;
            end

            if isequal(innerBound,'reflect') % absorbing or semi-absorbing outer boundary
                if d == 1
                    if isequal(model,'weighted')
                ...
                elseif d == 2
                    ...
                    if isequal(model,'weighted') % three-parameter model
    ...
    if isequal(model,'one-term') % single exponential model
...
if isequal(model,'one-term')
    s_exp = exp(-lambda*tc);
elseif isequal(model,'two-term')
    s_exp = (exp(-lambda(1)*tc) + exp(-lambda(2)*tc))/2;
else
    s_exp = theta*exp(-lambda(1)*tc) + (1-theta)*exp(-lambda(2)*tc);
end