...
if isequal(Case,'A')
    ...
    IB = struct('L0',0,'a0',0,'b0',1,'P0',0,'P1',0); % inner bound parameters (disc/sphere must have a reflecting inner "boundary")
    OB = struct('L1',100,'a1',1,'b1',0,'P0',0,'P1',0); % outer bound parameters (absorbing outer boundary)
elseif isequal(Case,'B')
    P = 1; % probability of movement
    P1 = 0.5; % probability of particle absorption at outer boundary
    IB = struct('L0',0,'a0',0,'b0',1); % inner bound parameters (disc/sphere must have a reflecting inner "boundary")

    if d == 2
        b1 = delta / (P1 + pi*delta); % contribution of outer boundary flux term for disc
        OB = struct('L1',100,'a1',1,'b1',b1,'P0',0,'P1',P1); % outer bound parameters (semi-absorbing outer boundary with P1 = 0.5)
    else
...
elseif isequal(Case,'C')
    P = 1; % probability of movement
    IB = struct('L0',50,'a0',0,'b0',1,'P0',0,'P1',0); % inner bound parameters (reflecting inner boundary)
    OB = struct('L1',100,'a1',1,'b1',0,'P0',0,'P1',0); % outer bound parameters (absorbing outer boundary)
elseif isequal(Case,'D')
    P = 1; % probability of movement
    P1 = 0.5; % probability of particle absorption at outer boundary
    IB = struct('L0',50,'a0',0,'b0',1); % lower bound parameters (reflecting inner boundary)
...
elseif isequal(Case,'E')
    P = 1; % probability of movement
    IB = struct('L0',50,'a0',1,'b0',0,'P0',0,'P1',0); % inner bound parameters (absorbing inner boundary)
    OB = struct('L1',100,'a1',1,'b1',0,'P0',0,'P1',0); % outer bound parameters (absorbing outer boundary)
elseif isequal(Case,'F')
    ...
    if d == 2
        ...
        IB = struct('L0',0,'a0',1,'b0',b0,'P0',P0,'P1',0); % outer bound parameters (semi-absorbing outer boundary with P1 = 0.5)
    else
...
elseif isequal(Case,'G')
    ...
    IB = struct('L0',0,'a0',0,'b0',1,'P0',0,'P1',0); % inner bound parameters (reflecting inner boundary)
    IF = 40; % interface point
    OB = struct('L2',100,'a1',1,'b1',0,'P0',0,'P1',0); % outer bound parameters (absorbing outer boundary)
elseif isequal(Case,'H')
    P = [0.4,1]; % probability of movement in each layer
    P1 = 0.5; % probability of particle absorption at the outer boundary
    IB = struct('L0',0,'a0',0,'b0',1); % inner bound parameters (reflecting inner boundary)
    IF = 40; % interface point
